...
class DBN(object):
    def __init__(self, numpy_rng, n_ins,
        ...
        self.FinalLayer = HiddenLayer(rng=numpy_rng,
        ...
        # be small
        self.L1 = abs(self.FinalLayer.W).sum()
        for i in range(2 * self.n_layers)[0::2]:
            self.L1 += abs(self.params[i]).sum()

        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        self.L2_sqr = (self.FinalLayer.W ** 2).sum()
        for i in range(2 * self.n_layers)[0::2]:
            self.L2_sqr += (self.params[i] ** 2).sum()

        self.squared_errors = self.FinalLayer.squared_errors

        self.finetune_cost = self.squared_errors(self.y) + L1_reg * self.L1 + L2_reg * self.L2_sqr

        self.params.extend(self.FinalLayer.params)
        self.input = input

        self.y_pred = self.FinalLayer.output
...
def test_DBN(finetune_lr, pretraining_epochs, pretrain_lr, k, training_epochs,
    ...
    while (epoch < training_epochs) and (not done_looping):
        ...
        for minibatch_index in range(int(n_train_batches)):
            ...
            minibatch_finetune_cost, minibatch_mse_cost = train_fn(minibatch_index)
